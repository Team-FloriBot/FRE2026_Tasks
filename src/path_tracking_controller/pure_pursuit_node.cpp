...
public:
    PurePursuitNode() : Node("pure_pursuit_node")
    {
        ...
        this->declare_parameter<double>("pure_pursuit_gain", 1.0);
        ...
        this->declare_parameter<double>("projection_search_distance", 1.5);
        this->declare_parameter<double>("projection_backtrack_distance", 0.20);
        this->declare_parameter<bool>("heading_filter_enabled", true);
        this->declare_parameter<double>("heading_tolerance", 1.75);
        this->declare_parameter<double>("min_follow_turn_radius", 0.37);
...
private:
    ...
    double curvature_gain_, pure_pursuit_gain_, max_angular_velocity_;
    ...
    ProjectionResult project_robot_to_path(const geometry_msgs::msg::PoseStamped & robot)
    {
        ...
        {
            start_s = std::max(0.0, progress_s_ - std::max(0.0, projection_backtrack_distance_));
            end_s = std::min(
                path_length_,
        ...
        if (end_s < start_s + 1e-6)
            end_s = std::min(path_length_, start_s + std::max(projection_search_distance_, lookahead_min_));
        ...
        for (size_t i = 0; i < path_.poses.size() - 1; i++)
        {
            ...
            double raw_t = ((robot_x - p0.x) * dx + (robot_y - p0.y) * dy) / seg_len_sq;
            ...
            candidate.heading_error = std::abs(normalize_angle(segment_yaw - robot_yaw));
            candidate.heading_ok = !heading_filter_enabled_ ||
    ...
    void path_callback(const nav_msgs::msg::Path::SharedPtr msg)
    {
        ...
        if (!build_path_metrics())
        {
            ...
            tracking_enabled_ = false;
            path_completed_ = true;
            RCLCPP_WARN(get_logger(), "Empfangener Pfad hat keine nutzbare Laenge. Stoppe Path Tracking.");
            return;
        }
                       ...
                       size_t segment_idx)
    {
        ...
        auto sphere = [&](int id, double x, double y,
        ...
        auto line = [&](int id, const std::vector<geometry_msgs::msg::Point> & points,
                        float r, float g, float b, double width)
        {
            visualization_msgs::msg::Marker m;
            m.header.frame_id = path_.header.frame_id;
            m.header.stamp = this->now();
        ...
        auto text = [&](int id, const geometry_msgs::msg::Point & p, const std::string & value)
        {
            visualization_msgs::msg::Marker m;
            m.header.frame_id = path_.header.frame_id;
            m.header.stamp = this->now();
        ...
        arr.markers.push_back(sphere(0, robot.pose.position.x, robot.pose.position.y, 1.0, 0.0, 0.0, 0.22));
        arr.markers.push_back(sphere(1, projection.x, projection.y, 1.0, 0.85, 0.0, 0.18));
        arr.markers.push_back(sphere(2, lookahead.x, lookahead.y, 0.0, 1.0, 0.0, 0.22));
        ...
        arr.markers.push_back(line(3, pursuit_line, 0.0, 1.0, 0.0, 0.035));
        ...
        if (path_.poses.size() >= 2)
        {
            ...
            arr.markers.push_back(line(4, segment_line, 0.0, 0.6, 1.0, 0.06));
        }
    ...
    void control_loop()
    {
        ...
        double curvature = pure_pursuit_gain_ * 2.0 * std::sin(alpha) / dist;
        ...
        {
            double radius_limited_omega = std::abs(v) / min_follow_turn_radius_;
            max_omega = std::min(max_omega, radius_limited_omega);
        }

        double omega = std::clamp(v * curvature, -max_omega, max_omega);
